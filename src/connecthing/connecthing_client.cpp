...
bool ConnecthingClient::ParseURL(const std::string& url,
                                 URLParts& parts,
                                 bool& ssl) const {
...
ConnecthingClient::GetRTCConnectionState() const {
  return rtc_state_;
}

std::shared_ptr<RTCConnection> ConnecthingClient::GetRTCConnection() const {
...
void ConnecthingClient::GetStats(
    std::function<void(
...
ConnecthingClient::ConnecthingClient(boost::asio::io_context& ioc,
                                     RTCManager* manager,
    ...
    : ioc_(ioc),
      ...
      timer_(ioc),
      watchdog_(ioc, std::bind(&ConnecthingClient::OnWatchdogExpired, this)) {
  Reset();
}

ConnecthingClient::~ConnecthingClient() {
  destructed_ = true;
  // 一応閉じる努力はする
...
void ConnecthingClient::Close(std::function<void()> on_close) {
...
void ConnecthingClient::Reset() {
  watchdog_.Disable();
  connection_ = nullptr;

  connecting_wss_.clear();
  ws_.reset();
  dc_.reset(new ConnecthingDataChannelOnAsio(ioc_, this));
}

void ConnecthingClient::Connect() {
  ...
  for (const auto& url : config_.signaling_urls) {
    ...
    RTC_LOG(LS_INFO) << __FUNCTION__ << "-----scheme:" << parts.scheme;
    RTC_LOG(LS_INFO) << __FUNCTION__ << "-----user_pass:" << parts.user_pass;
    RTC_LOG(LS_INFO) << __FUNCTION__ << "-----host:" << parts.host;
    RTC_LOG(LS_INFO) << __FUNCTION__ << "-----port:" << parts.port;
    RTC_LOG(LS_INFO) << __FUNCTION__ << "-----path_query_fragment:"
    ...
    ws->Connect(url,
                std::bind(&ConnecthingClient::OnConnect, shared_from_this(),
                          std::placeholders::_1, url, ws));
    connecting_wss_.push_back(ws);
  }
}

void ConnecthingClient::ReconnectAfter() {
...
void ConnecthingClient::OnWatchdogExpired() {
...
void ConnecthingClient::OnConnect(boost::system::error_code ec,
                                  std::string url,
                                  std::shared_ptr<Websocket> ws) {
  RTC_LOG(LS_INFO) << __FUNCTION__ << "-----OnConnect-url:" << url;
  RTC_LOG(LS_INFO) << __FUNCTION__
                   << "-----OnConnect-connecting_wss_.size()before:"
                   << connecting_wss_.size();
  connecting_wss_.erase(
      std::remove_if(connecting_wss_.begin(), connecting_wss_.end(),
                     [ws](std::shared_ptr<Websocket> p) { return p == ws; }),
      connecting_wss_.end());
  RTC_LOG(LS_INFO) << __FUNCTION__
                   << "-----OnConnect-connecting_wss_.size()after:"
                   << connecting_wss_.size();
  if (ec) {
    // すべての接続がうまくいかなかったら再接続フローに入る
    if (!ws_ && connecting_wss_.empty()) {
      RTC_LOG(LS_INFO) << __FUNCTION__ << "-----Try ReconnectAfter";
      ReconnectAfter();
    }
...
void ConnecthingClient::Redirect(std::string url) {
  ws_->Read([self = shared_from_this(), url](boost::system::error_code ec,
    ...
    auto on_close = [self, url](boost::system::error_code ec) {
      ...
      ws->Connect(url, std::bind(&ConnecthingClient::OnRedirect, self,
                                 std::placeholders::_1, url, ws));
    };
...
void ConnecthingClient::OnRedirect(boost::system::error_code ec,
                                   std::string url,
                                   std::shared_ptr<Websocket> ws) {
  if (ec) {
    ReconnectAfter();
...
void ConnecthingClient::DoSendConnect(bool redirect) {
  boost::json::object json_message = {
      {"type", "connect"},
      {"role", config_.role},
      {"channel_id", config_.channel_id},
      {"connecthing_client", MomoVersion::GetClientName()},
...
void ConnecthingClient::DoSendPong() {
...
void ConnecthingClient::DoSendUpdate(const std::string& sdp, std::string type) {
...
// std::shared_ptr<RTCConnection> ConnecthingClient::CreateRTCConnection(
...
void ConnecthingClient::OnRead(boost::system::error_code ec,
                               std::size_t bytes_transferred,
                               std::string text) {
  ...
  */
  auto json_message = boost::json::parse(text);
  std::string janus = json_message.at("janus").as_string().c_str();
  RTC_LOG(LS_INFO) << __FUNCTION__ << "-----janus: " << janus;
  if (janus == "success") {
    if (session_id == -1) {
      RTC_LOG(LS_INFO) << __FUNCTION__ << "-----session_id is empty";
      session_id = json_message.at("data").at("id").as_int64();
      RTC_LOG(LS_INFO) << __FUNCTION__ << "-----session_id: " << session_id;
      AttachPlugin();
    } else {
      if (json_message.at("session_id").as_int64() == session_id) {
        handle_id = json_message.at("data").at("id").as_int64();
        RTC_LOG(LS_INFO) << __FUNCTION__ << "-----handle_id: " << handle_id;
        JoinRoomAsPublisher();
      }
    }
  } else if (janus == "event") {
    std::string videoroom = json_message.at("plugindata")
                                .at("data")
                                .at("videoroom")
                                .as_string()
                                .c_str();
    RTC_LOG(LS_INFO) << __FUNCTION__ << "-----videoroom: " << videoroom;
    if (videoroom == "joined") {
      private_id =
          json_message.at("plugindata").at("data").at("private_id").as_int64();
      RTC_LOG(LS_INFO) << __FUNCTION__ << "-----private_id: " << private_id;
      CreateOffer();
      RTC_LOG(LS_INFO) << __FUNCTION__ << "-----Complete create offer";
    } else if (videoroom == "event") {
      RTC_LOG(LS_INFO) << __FUNCTION__ << "-----1";
      if (auto p = json_message.at("plugindata")
                       .at("data")
                       .as_object()
                       .if_contains("configured")) {
        std::string configured = json_message.at("plugindata")
                                     .at("data")
                                     .at("configured")
                                     .as_string()
                                     .c_str();
        RTC_LOG(LS_INFO) << __FUNCTION__ << "-----configured: " << configured;
        if (configured == "ok") {
          if (auto p = json_message.as_object().if_contains("jsep")) {
            std::string type =
                json_message.at("jsep").at("type").as_string().c_str();
            RTC_LOG(LS_INFO) << __FUNCTION__ << "-----2";
            if (type == "answer") {
              std::string sdp =
                  json_message.at("jsep").at("sdp").as_string().c_str();
              connection_->SetAnswer(sdp);
              RTC_LOG(LS_INFO) << __FUNCTION__ << "-----Complete set answer";
            }
          }
        }
      }
    ...
    std::string sdp_mid =
        json_message.at("candidate").at("sdpMid").as_string().c_str();
    int sdp_mlineindex =
        json_message.at("candidate").at("sdpMLineIndex").to_number<int>();
    std::string candidate =
        json_message.at("candidate").at("candidate").as_string().c_str();
    if (!candidate.empty()) {
...
void ConnecthingClient::CreateSession() {
  transaction = Util::GenerateRandomChars();
  boost::json::object json_message = {
      {"janus", "create"},
...
void ConnecthingClient::AttachPlugin() {
  boost::json::object json_message = {
      {"janus", "attach"},        {"plugin", "janus.plugin.videoroom"},
      {"session_id", session_id}, {"transaction", transaction},
      {"apisecret", API_SECRET},
  };
  ws_->WriteText(boost::json::serialize(json_message));
}

void ConnecthingClient::JoinRoomAsPublisher() {
  milliseconds ms =
      duration_cast<milliseconds>(system_clock::now().time_since_epoch());
  RTC_LOG(LS_INFO) << __FUNCTION__ << "-----timestamp for id:" << ms.count();
  boost::json::object json_message = {{"janus", "message"},
                                      {"transaction", transaction},
                                      {"apisecret", API_SECRET},
...
void ConnecthingClient::CreateOffer() {
  ...
  if (connection_ == nullptr) {
    RTC_LOG(LS_INFO) << __FUNCTION__ << "-----connection_ is null";
  } else {
    RTC_LOG(LS_INFO) << __FUNCTION__ << "-----Complete create peerconnection";
    manager_->InitTracks(connection_.get(), std::nullopt);
    connection_->CreateOffer(on_create_offer);
  }
}

void ConnecthingClient::StartSendPing() {
  watchdog_.Enable(30);
  ...
  timer_.async_wait([this](const boost::system::error_code& ec) {
    ...
    ws_->WriteText(boost::json::serialize(json_message));
    // std::string label = cdc->label();
    // RTC_LOG(LS_INFO) << "-----Configured datachannel label:" << label;
    std::string random_value = Util::GenerateRandomChars(10);
    SendDataChannel(DEFAULT_DATA_CHANNEL_LABEL, random_value);
    RTC_LOG(LS_INFO) << "-----Sent datachannel random value:" << random_value;
    StartSendPing();
  });
}

webrtc::DataBuffer ConnecthingClient::ConvertToDataBuffer(
...
void ConnecthingClient::SendDataChannel(const std::string& label,
                                        const std::string& input) {
  webrtc::DataBuffer data = ConvertToDataBuffer(label, input);
  dc_->Send(label, data);
}

void ConnecthingClient::OnStateChange(
    webrtc::scoped_refptr<webrtc::DataChannelInterface> data_channel) {}

void ConnecthingClient::OnMessage(
    webrtc::scoped_refptr<webrtc::DataChannelInterface> data_channel,
...
void ConnecthingClient::OnIceConnectionStateChange(
  ...
  boost::asio::post(ioc_,
                    std::bind(&ConnecthingClient::DoIceConnectionStateChange,
                              shared_from_this(), new_state));
}
void ConnecthingClient::OnIceCandidate(const std::string sdp_mid,
                                       const int sdp_mlineindex,
                                       const std::string sdp) {
  // boost::json::value json_message = {{"type", "candidate"}, {"candidate", sdp}};
  boost::json::object json_message = {{"janus", "trickle"},
                                      {"transaction", transaction},
                                      {"apisecret", API_SECRET},
                                      {"session_id", session_id},
                                      {"handle_id", handle_id},
...
void ConnecthingClient::DoIceConnectionStateChange(
    webrtc::PeerConnectionInterface::IceConnectionState new_state) {

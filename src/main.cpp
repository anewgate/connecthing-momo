...
#include "connecthing/connecthing_client.h"
#include "connecthing/connecthing_server.h"
...
int main(int argc, char* argv[]) {
  ...
  bool use_sora = false;
  bool use_connecthing = false;
  int log_level = webrtc::LS_NONE;

  Util::ParseArgs(argc, argv, use_p2p, use_ayame, use_sora, use_connecthing,
                  log_level, args);
...

  {
    ...
    std::shared_ptr<ConnecthingClient> connecthing_client;
    ...
    if (use_connecthing) {
      ...
      config.insecure = args.insecure;
      config.signaling_urls = args.connecthing_signaling_urls;
      config.channel_id = args.connecthing_channel_id;
      config.video = args.connecthing_video;
      config.audio = args.connecthing_audio;
      config.video_codec_type = args.connecthing_video_codec_type;
      config.audio_codec_type = args.connecthing_audio_codec_type;
      config.video_bit_rate = args.connecthing_video_bit_rate;
      config.audio_bit_rate = args.connecthing_audio_bit_rate;
      config.metadata = args.connecthing_metadata;
      config.role = args.connecthing_role;
      config.spotlight = args.connecthing_spotlight;
      config.spotlight_number = args.connecthing_spotlight_number;
      config.port = args.connecthing_port;
      config.simulcast = args.connecthing_simulcast;
      config.data_channel_signaling = args.connecthing_data_channel_signaling;
      config.data_channel_signaling_timeout =
          args.connecthing_data_channel_signaling_timeout;
      config.ignore_disconnect_websocket =
          args.connecthing_ignore_disconnect_websocket;
      config.disconnect_wait_timeout = args.connecthing_disconnect_wait_timeout;
      config.client_cert = args.client_cert;
      config.client_key = args.client_key;
      config.proxy_url = args.proxy_url;
      config.proxy_username = args.proxy_username;
      ...
      connecthing_client =
          ConnecthingClient::Create(ioc, rtc_manager.get(), std::move(config));

      // ConnecthingServer を起動しない場合と、ConnecthingServer を起動して --auto が指定されている場合は即座に接続する。
      // ConnecthingServer を起動するけど --auto が指定されていない場合、ConnecthingServer の API が呼ばれるまで接続しない。
      if (args.connecthing_port < 0 ||
          args.connecthing_port >= 0 && args.connecthing_auto_connect) {
        connecthing_client->Connect();
      }
      ...
      //   const boost::asio::ip::tcp::endpoint endpoint{
      //       boost::asio::ip::make_address("127.0.0.1"),
      //       static_cast<unsigned short>(args.connecthing_port)};
      //   ConnecthingServer::Create(ioc, endpoint, connecthing_client, rtc_manager.get(),
      //                      config)
      //       ->Run();
      // }

      stats_collector = connecthing_client;
    }

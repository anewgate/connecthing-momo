...
struct MomoArgs {
  ...
  std::vector<std::string> connecthing_signaling_urls;
  std::string connecthing_channel_id;
  bool connecthing_video = true;
  bool connecthing_audio = true;
  // 空文字の場合コーデックは connecthing 側で決める
  std::string connecthing_video_codec_type = "";
  std::string connecthing_audio_codec_type = "";
  // 0 の場合ビットレートは connecthing 側で決める
  int connecthing_video_bit_rate = 0;
  int connecthing_audio_bit_rate = 0;
  bool connecthing_auto_connect = false;
  boost::json::value connecthing_metadata;
  // sendonly, recvonly, sendrecv
  std::string connecthing_role = "sendonly";
  bool connecthing_spotlight = false;
  int connecthing_spotlight_number = 0;
  int connecthing_port = -1;
  bool connecthing_simulcast = false;
  boost::optional<bool> connecthing_data_channel_signaling;
  int connecthing_data_channel_signaling_timeout = 180;
  boost::optional<bool> connecthing_ignore_disconnect_websocket;
  int connecthing_disconnect_wait_timeout = 5;

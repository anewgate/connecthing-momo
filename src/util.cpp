...
void Util::ParseArgs(int argc,
  ...
  auto connecthing_app = app.add_subcommand(
      "connecthing", "Mode for working with WebRTC SFU Connecthing");
  ...
  connecthing_app
      ->add_option("--signaling-urls", args.connecthing_signaling_urls,
                   "Signaling URLs")
      ->take_all()
      ->required();
  connecthing_app
      ->add_option("--channel-id", args.connecthing_channel_id, "Channel ID")
      ->required();
  connecthing_app->add_flag("--auto", args.connecthing_auto_connect,
                            "Connect to Connecthing automatically");

  connecthing_app
      ->add_option("--video", args.connecthing_video,
                   "Send video to connecthing (default: true)")
      ->transform(CLI::CheckedTransformer(bool_map, CLI::ignore_case));
  connecthing_app
      ->add_option("--audio", args.connecthing_audio,
                   "Send audio to connecthing (default: true)")
      ->transform(CLI::CheckedTransformer(bool_map, CLI::ignore_case));
  connecthing_app
      ->add_option("--video-codec-type", args.connecthing_video_codec_type,
                   "Video codec for send")
      ->check(CLI::IsMember({"", "VP8", "VP9", "AV1", "H264", "H265"}));
  connecthing_app
      ->add_option("--audio-codec-type", args.connecthing_audio_codec_type,
                   "Audio codec for send")
      ->check(CLI::IsMember({"", "OPUS"}));
  connecthing_app
      ->add_option("--video-bit-rate", args.connecthing_video_bit_rate,
                   "Video bit rate")
      ->check(CLI::Range(0, 30000));
  connecthing_app
      ->add_option("--audio-bit-rate", args.connecthing_audio_bit_rate,
                   "Audio bit rate")
      ->check(CLI::Range(0, 510));
  connecthing_app
      ->add_option("--role", args.connecthing_role, "Role (default: sendonly)")
      ->check(CLI::IsMember({"sendonly", "recvonly", "sendrecv"}));
  connecthing_app
      ->add_option("--spotlight", args.connecthing_spotlight, "Use spotlight")
      ->transform(CLI::CheckedTransformer(bool_map, CLI::ignore_case));
  connecthing_app
      ->add_option("--spotlight-number", args.connecthing_spotlight_number,
                   "Stream count delivered in spotlight")
      ->check(CLI::Range(0, 8));
  connecthing_app
      ->add_option("--port", args.connecthing_port, "Port number (default: -1)")
      ->check(CLI::Range(-1, 65535));
  connecthing_app
      ->add_option("--simulcast", args.connecthing_simulcast,
                   "Use simulcast (default: false)")
      ->transform(CLI::CheckedTransformer(bool_map, CLI::ignore_case));
  add_optional_bool(
      connecthing_app, "--data-channel-signaling",
      args.connecthing_data_channel_signaling,
      "Use DataChannel for Connecthing signaling (default: none)");
  connecthing_app
      ->add_option("--data-channel-signaling-timeout",
                   args.connecthing_data_channel_signaling_timeout,
                   "Timeout for Data Channel in seconds (default: 180)")
      ->check(CLI::PositiveNumber);
  add_optional_bool(connecthing_app, "--ignore-disconnect-websocket",
                    args.connecthing_ignore_disconnect_websocket,
                    "Ignore WebSocket disconnection if using Data Channel "
                    "(default: none)");
  connecthing_app
      ->add_option(
          "--disconnect-wait-timeout", args.connecthing_disconnect_wait_timeout,
          "Disconnecting timeout for Data Channel in seconds (default: 5)")
      ->check(CLI::PositiveNumber);

  // auto is_json = CLI::Validator(
  auto is_json_connecthing = CLI::Validator(
      [](std::string input) -> std::string {
        boost::system::error_code ec;
        boost::json::parse(input, ec);
        if (ec) {
      ...
      "JSON Value");
  std::string connecthing_metadata;
  connecthing_app
      ->add_option("--metadata", connecthing_metadata,
                   "Signaling metadata used in connect message")
      // ->check(is_json);
      ->check(is_json_connecthing);
  ...
  if (!connecthing_metadata.empty()) {
    args.connecthing_metadata = boost::json::parse(connecthing_metadata);
  }
  ...
  if (connecthing_app->parsed()) {
    use_connecthing = true;
  }
